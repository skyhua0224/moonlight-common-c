...
// This function returns a string that you SHOULD append to the /launch and
// /resume query parameter string. This is used to enable certain extended
// functionality with Sunshine hosts. The returned string is owned by
// moonlight-common-c and should not be freed by the caller.
...
// This is a key frame.
//
// For H.264 and HEVC, this means the frame contains SPS, PPS, and VPS (HEVC
// only) NALUs as the first buffers in the list. The I-frame data follows
// immediately after the codec configuration NALUs.
//
...
typedef struct _DECODE_UNIT {
  ...
  // Receive time of first buffer. This value uses an implementation-defined
  // epoch, but the same epoch as enqueueTimeMs and LiGetMillis().
  uint64_t receiveTimeMs;

  // Time the frame was fully assembled and queued for the video decoder to
  // process. This is also approximately the same time as the final packet was
  // received, so enqueueTimeMs - receiveTimeMs is the time taken to receive the
  // frame. At the time the decode unit is passed to submitDecodeUnit(), the
  // total queue delay can be calculated by LiGetMillis() - enqueueTimeMs.
  uint64_t enqueueTimeMs;

  // Presentation time in milliseconds with the epoch at the first captured
  // frame. This can be used to aid frame pacing or to drop old frames that were
  // queued too long prior to display.
...
// Specifies that the audio stream should be in 5.1 surround sound if the PC is
// able
#define AUDIO_CONFIGURATION_51_SURROUND MAKE_AUDIO_CONFIGURATION(6, 0x3F)

// Specifies that the audio stream should be in 7.1 surround sound if the PC is
// able
...
// for channelMask values NOTE: Not all combinations are supported by GFE and/or
// this library.
#define MAKE_AUDIO_CONFIGURATION(channelCount, channelMask)                    \
  (((channelMask) << 16) | (channelCount << 8) | 0xCA)

// Helper macros for retreiving channel count and channel mask from the audio
// configuration
#define CHANNEL_COUNT_FROM_AUDIO_CONFIGURATION(x) (((x) >> 8) & 0xFF)
#define CHANNEL_MASK_FROM_AUDIO_CONFIGURATION(x) (((x) >> 16) & 0xFFFF)

// Helper macro to retreive the surroundAudioInfo parameter value that must be
// passed in the /launch and /resume HTTPS requests when starting the session.
#define SURROUNDAUDIOINFO_FROM_AUDIO_CONFIGURATION(x)                          \
  (CHANNEL_MASK_FROM_AUDIO_CONFIGURATION(x) << 16 |                            \
...
// Masks for clients to use to match video codecs without profile-specific
// details.
...
// If set in the renderer capabilities field, this flag will cause audio/video
// data to be submitted directly from the receive thread. This should only be
// specified if the renderer is non-blocking. This flag is valid on both audio
// and video renderers.
#define CAPABILITY_DIRECT_SUBMIT 0x1

// If set in the video renderer capabilities field, this flag specifies that the
// renderer supports reference frame invalidation for AVC/H.264 streams. This
// flag is only valid on video renderers. If using this feature, the bitstream
// may not be patched (changing num_ref_frames or max_dec_frame_buffering) to
// avoid video corruption on packet loss.
#define CAPABILITY_REFERENCE_FRAME_INVALIDATION_AVC 0x2

// If set in the video renderer capabilities field, this flag specifies that the
// renderer supports reference frame invalidation for HEVC/H.265 streams. This
// flag is only valid on video renderers.
#define CAPABILITY_REFERENCE_FRAME_INVALIDATION_HEVC 0x4

// If set in the audio renderer capabilities field, this flag will cause the
// RTSP negotiation to never request the "high quality" audio preset. If unset,
// high quality audio will be used with video streams above 15 Mbps.
#define CAPABILITY_SLOW_OPUS_DECODER 0x8

// If set in the audio renderer capabilities field, this indicates that audio
// packets may contain more or less than 5 ms of audio. This requires that audio
// renderers read the samplesPerFrame field in OPUS_MULTISTREAM_CONFIGURATION to
// calculate the correct decoded buffer size rather than just assuming it will
// always be 240.
#define CAPABILITY_SUPPORTS_ARBITRARY_AUDIO_DURATION 0x10

// This flag opts the renderer into a pull-based model rather than the default
// push-based callback model. The renderer must invoke the new functions
// (LiWaitForNextVideoFrame(), LiCompleteVideoFrame(), and similar) to receive
// A/V data. Setting this capability while also providing a sample callback is
// not allowed.
#define CAPABILITY_PULL_RENDERER 0x20

// If set in the video renderer capabilities field, this flag specifies that the
// renderer supports reference frame invalidation for AV1 streams. This flag is
// only valid on video renderers.
#define CAPABILITY_REFERENCE_FRAME_INVALIDATION_AV1 0x40

// If set in the video renderer capabilities field, this macro specifies that
// the renderer supports slicing to increase decoding performance. The parameter
// specifies the desired number of slices per frame. This capability is only
// valid on video renderers.
#define CAPABILITY_SLICES_PER_FRAME(x) (((unsigned char)(x)) << 24)

// This callback is invoked to provide details about the video stream and allow
// configuration of the decoder. Returns 0 on success, non-zero on failure.
typedef int (*DecoderRendererSetup)(int videoFormat, int width, int height,
                                    int redrawRate, void *context, int drFlags);

// This callback notifies the decoder that the stream is starting. No frames can
// be submitted before this callback returns.
typedef void (*DecoderRendererStart)(void);

// This callback notifies the decoder that the stream is stopping. Frames may
// still be submitted but they may be safely discarded.
typedef void (*DecoderRendererStop)(void);

// This callback performs the teardown of the video decoder. No more frames will
// be submitted when this callback is invoked.
typedef void (*DecoderRendererCleanup)(void);

// This callback provides Annex B formatted elementary stream data to the
// decoder. If the decoder is unable to process the submitted data for some
// reason, it must return DR_NEED_IDR to generate a keyframe.
#define DR_OK 0
...
// Use this function to zero the video callbacks when allocated on the stack or
// heap
void LiInitializeVideoCallbacks(PDECODER_RENDERER_CALLBACKS drCallbacks);

// This structure provides the Opus multistream decoder parameters required to
// successfully decode the audio stream being sent from the computer. See
// opus_multistream_decoder_init docs for details about these fields.
//
// The supplied mapping array is indexed according to the following output
// channel order: 0 - Front Left 1 - Front Right 2 - Center 3 - LFE 4 - Back
// Left 5 - Back Right 6 - Side Left 7 - Side Right
//
// If the mapping order does not match the channel order of the audio renderer,
// you may swap the values in the mismatched indices until the mapping array
// matches the desired channel order.
...
// This callback initializes the audio renderer. The audio configuration
// parameter provides the negotiated audio configuration. This may differ from
// the one specified in the stream configuration. Returns 0 on success, non-zero
// on failure.
typedef int (*AudioRendererInit)(
    int audioConfiguration, const POPUS_MULTISTREAM_CONFIGURATION opusConfig,
    void *context, int arFlags);

// This callback notifies the decoder that the stream is starting. No audio can
// be submitted before this callback returns.
typedef void (*AudioRendererStart)(void);

// This callback notifies the decoder that the stream is stopping. Audio samples
// may still be submitted but they may be safely discarded.
typedef void (*AudioRendererStop)(void);

// This callback performs the final teardown of the audio decoder. No additional
// audio will be submitted when this callback is invoked.
typedef void (*AudioRendererCleanup)(void);

// This callback provides Opus audio data to be decoded and played. sampleLength
// is in bytes.
typedef void (*AudioRendererDecodeAndPlaySample)(char *sampleData,
                                                 int sampleLength);
...
// Use this function to zero the audio callbacks when allocated on the stack or
// heap
...
// This callback is invoked to indicate that a stage of initialization has
// completed
typedef void (*ConnListenerStageComplete)(int stage);

// This callback is invoked to indicate that a stage of initialization has
// failed. ConnListenerConnectionTerminated() will not be invoked because the
// connection was not yet fully established. LiInterruptConnection() and
// LiStopConnection() may result in this callback being invoked, but it is not
// guaranteed.
...
// This error code is passed to ConnListenerConnectionTerminated() when the
// stream is being gracefully terminated by the host. It usually means the app
// on the host PC has exited.
#define ML_ERROR_GRACEFUL_TERMINATION 0

// This error is passed to ConnListenerConnectionTerminated() if no video data
// was ever received for this connection after waiting several seconds. It
// likely indicates a problem with traffic on UDP 47998 due to missing or
...
// frame could not be received after waiting several seconds. It likely
// indicates an extremely unstable connection or a bitrate that is far too high.
...
// another issue that prevents the encoder from being able to capture video
// successfully.
#define ML_ERROR_UNEXPECTED_EARLY_TERMINATION -102

// This error is passed to ConnListenerConnectionTerminated() if the stream ends
// due to a protected content error from the host. This value is supported on
// GFE 3.22+.
...
typedef void (*ConnListenerRumble)(unsigned short controllerNumber,
...
typedef void (*ConnListenerRumbleTriggers)(uint16_t controllerNumber,
...
typedef void (*ConnListenerSetMotionEventState)(uint16_t controllerNumber,
                                                uint8_t motionType,
...
typedef void (*ConnListenerSetAdaptiveTriggers)(uint16_t controllerNumber,
                                                uint8_t eventFlags,
                                                uint8_t typeLeft,
                                                uint8_t typeRight,
...
// stack or heap
void LiInitializeConnectionCallbacks(
...
#define SCM_MASK_HEVC                                                          \
  (SCM_HEVC | SCM_HEVC_MAIN10 | SCM_HEVC_REXT8_444 | SCM_HEVC_REXT10_444)
#define SCM_MASK_AV1                                                           \
  (SCM_AV1_MAIN8 | SCM_AV1_MAIN10 | SCM_AV1_HIGH8_444 | SCM_AV1_HIGH10_444)
#define SCM_MASK_10BIT                                                         \
  (SCM_HEVC_MAIN10 | SCM_HEVC_REXT10_444 | SCM_AV1_MAIN10 | SCM_AV1_HIGH10_444)
#define SCM_MASK_YUV444                                                        \
  (SCM_H264_HIGH8_444 | SCM_HEVC_REXT8_444 | SCM_HEVC_REXT10_444 |             \
...
// Use this function to zero the server information when allocated on the stack
// or heap
void LiInitializeServerInformation(PSERVER_INFORMATION serverInfo);

// This function begins streaming.
//
// Callbacks are all optional. Pass NULL for individual callbacks within each
// struct or pass NULL for the entire struct to use the defaults for all
// callbacks.
//
...
int LiStartConnection(PSERVER_INFORMATION serverInfo,
                      ...
                      void *renderContext, int drFlags, void *audioContext,
                      int arFlags);

// This function stops streaming. This function is not thread-safe.
void LiStopConnection(void);

// This function interrupts a pending LiStartConnection() call. This
// interruption happens asynchronously so it is not safe to start another
// connection before the first LiStartConnection() call returns.
...
// This function returns an estimate of the current RTT to the host PC obtained
// via ENet protocol statistics. This function will fail if the current GFE
// version does not use ENet for the control stream (very old versions), or if
// the ENet peer is not connected. This function may only be called between
// LiStartConnection() and LiStopConnection().
bool LiGetEstimatedRttInfo(uint32_t *estimatedRtt,
                           uint32_t *estimatedRttVariance);

// This function queues a relative mouse move event to be sent to the remote
// server.
int LiSendMouseMoveEvent(short deltaX, short deltaY);

// This function queues a mouse position update event to be sent to the remote
// server. This functionality is only reliably supported on GFE 3.20 or later.
// Earlier versions may not position the mouse correctly.
//
// Absolute mouse motion doesn't work in many games, so this mode should not be
// the default for mice when streaming. It may be desirable as the default
// touchscreen behavior when LiSendTouchEvent() is not supported and the
// touchscreen is not the primary input method. In the latter case, a
// touchscreen-as-trackpad mode using LiSendMouseMoveEvent() is likely to be
// better for gaming use cases.
//
// The x and y values are transformed to host coordinates as if they are from a
// plane which is referenceWidth by referenceHeight in size. This allows you to
// provide coordinates that are relative to an arbitrary plane, such as a
// window, screen, or scaled video view.
//
// For example, if you wanted to directly pass window coordinates as x and y,
// you would set referenceWidth and referenceHeight to your window width and
// height.
int LiSendMousePositionEvent(short x, short y, short referenceWidth,
                             short referenceHeight);

// This function queues a mouse position update event to be sent to the remote
// server, so all of the limitations of LiSendMousePositionEvent() mentioned
// above apply here too!
//
// This function behaves like a combination of LiSendMouseMoveEvent() and
// LiSendMousePositionEvent() in that it sends a relative motion event, however
// it sends this data as an absolute position based on the computed position of
// a virtual client cursor which is "moved" any time that
// LiSendMousePositionEvent() or LiSendMouseMoveAsMousePositionEvent() is
// called. As a result of this internal virtual cursor state, callers must
// ensure LiSendMousePositionEvent() and LiSendMouseMoveAsMousePositionEvent()
// are not called concurrently!
//
// The big advantage of this function is that it allows callers to avoid mouse
// acceleration that would otherwise affect motion when using
// LiSendMouseMoveEvent(). The downside is that it has the same game
// compatibility issues as LiSendMousePositionEvent().
//
// This function can be useful when mouse capture is the only feasible way to
// receive mouse input, like on Android or iOS, and the OS cannot provide raw
// unaccelerated mouse motion when capturing. Using this function avoids
// double-acceleration in cases when the client motion is also accelerated.
int LiSendMouseMoveAsMousePositionEvent(short deltaX, short deltaY,
                                        short referenceWidth,
                                        short referenceHeight);

// Error return value to indicate that the requested functionality is not
// supported by the host
#define LI_ERR_UNSUPPORTED -5501

// This function allows multi-touch input to be sent directly to Sunshine hosts.
// The x and y values are normalized device coordinates stretching top-left
// corner (0.0, 0.0) to bottom-right corner (1.0, 1.0) of the video area.
//
// Pointer ID is an opaque ID that must uniquely identify each active touch on
// screen. It must remain constant through any down/up/move/cancel events
// involved in a single touch interaction.
//
// Rotation is in degrees from vertical in Y dimension (parallel to screen,
// 0..360). If rotation is unknown, pass LI_ROT_UNKNOWN.
//
// Pressure is a 0.0 to 1.0 range value from min to max pressure. Sending a
// down/move event with a pressure of 0.0 indicates the actual pressure is
// unknown.
//
// For hover events, the pressure value is treated as a 1.0 to 0.0 range of
// distance from the touch surface where 1.0 is the farthest measurable distance
// and 0.0 is actually touching the display (which is invalid for a hover
// event). Reporting distance 0.0 for a hover event indicates the actual
// distance is unknown.
//
// Contact area is modelled as an ellipse with major and minor axis values in
// normalized device coordinates. If contact area is unknown, report 0.0 for
// both contact area axis parameters. For circular contact areas or if a minor
// axis value is not available, pass the same value for major and minor axes.
// For APIs or devices, that don't report contact area as an ellipse,
// approximations can be used such as:
// https://docs.kernel.org/input/multi-touch-protocol.html#event-computation
//
// For hover events, the "contact area" is the size of the hovering finger/tool.
// If unavailable, pass 0.0 for both contact area parameters.
//
// Touches can be cancelled using LI_TOUCH_EVENT_CANCEL or
// LI_TOUCH_EVENT_CANCEL_ALL. When using LI_TOUCH_EVENT_CANCEL, only the
// pointerId parameter is valid. All other parameters are ignored. To cancel all
// active touches (on focus loss, for example), use LI_TOUCH_EVENT_CANCEL_ALL.
//
// If unsupported by the host, this will return LI_ERR_UNSUPPORTED and the
// caller should consider falling back to other functions to send this input
// (such as LiSendMousePositionEvent()).
//
// To determine if LiSendTouchEvent() is supported without calling it, call
// LiGetHostFeatureFlags() and check for the LI_FF_PEN_TOUCH_EVENTS flag.
...
// This function is similar to LiSendTouchEvent() but allows additional
// parameters relevant for pen input, including tilt and buttons. Tilt is in
// degrees from vertical in Z dimension (perpendicular to screen, 0..90). See
// LiSendTouchEvent() for detailed documentation on other parameters.
//
// x, y, pressure, rotation, contact area, and tilt are ignored for
// LI_TOUCH_EVENT_BUTTON_ONLY events. If one of those changes, send
// LI_TOUCH_EVENT_MOVE or LI_TOUCH_EVENT_HOVER instead.
//
// To determine if LiSendPenEvent() is supported without calling it, call
// LiGetHostFeatureFlags() and check for the LI_FF_PEN_TOUCH_EVENTS flag.
...
// that the keycode was not mapped to a standard US English scancode and should
// be interpreted as-is. This is a Sunshine protocol extension.
...
#define MISC_FLAG                                                              \
  0x200000 // Share/Mic/Capture/Mute buttons on various controllers

// This function queues a controller event to be sent to the remote server. It
// will be seen by the computer as the first controller.
...
// This function queues a controller event to be sent to the remote server. The
// controllerNumber parameter is a zero-based index of which controller this
// event corresponds to. The largest legal controller number is 3 for GFE hosts
// and 15 for Sunshine hosts. On generation 3 servers (GFE 2.1.x), these will be
// sent as controller 0 regardless of the controllerNumber parameter.
//
// The activeGamepadMask parameter is a bitfield with bits set for each
// controller present. On GFE, activeGamepadMask is limited to a maximum of 4
// bits (0xF). On Sunshine, it is limited to 16 bits (0xFFFF).
//
// To indicate arrival of a gamepad, you may send an empty event with the
// controller number set to the new controller and the bit of the new controller
// set in the active gamepad mask. However, you should prefer
// LiSendControllerArrivalEvent() instead of this function for that purpose,
// because it allows the host to make a better choice of emulated controller.
//
// To indicate removal of a gamepad, send an empty event with the controller
// number set to the removed controller and the bit of the removed controller
// cleared in the active gamepad mask.
...
// This function provides a method of informing the host of the available
// buttons and capabilities on a new controller. This is the recommended
// approach for indicating the arrival of a new controller.
//
// This can allow the host to make better decisions about what type of
// controller to emulate and what capabilities to advertise to the OS on the
// virtual controller.
//
// If controller arrival events are unsupported by the host, this will fall back
// to indicating arrival via LiSendMultiControllerEvent().
...
//
// If unsupported by the host, this will return LI_ERR_UNSUPPORTED and the
// caller should consider using this touch input to simulate trackpad input.
//
// To determine if LiSendControllerTouchEvent() is supported without calling it,
// call LiGetHostFeatureFlags() and check for the LI_FF_CONTROLLER_TOUCH_EVENTS
// flag.
...
// ConnListenerSetMotionEventState().
//
// LI_MOTION_TYPE_ACCEL should report data in m/s^2 (inclusive of gravitational
// acceleration). LI_MOTION_TYPE_GYRO should report data in deg/s.
//
...
// This function allows clients to send controller battery state to a supported
// host. If the host can adjust battery state on the emulated controller, it can
// use this information to make the virtual controller match the physical
...
#define LI_BATTERY_STATE_NOT_CHARGING                                          \
  0x04 // Connected to power but not charging
...
// This is a simplistic STUN function that can assist clients in getting the WAN
// address for machines they find using mDNS over IPv4. This can be used to
// pre-populate the external address for streaming after GFE stopped sending it
// a while back. wanAddr is returned in network byte order.
int LiFindExternalAddressIP4(const char *stunServer, unsigned short stunPort,
...
// Port index flags for use with LiGetPortFromPortFlagIndex() and
// LiGetProtocolFromPortFlagIndex()
...
// Returns the port flags that correspond to ports involved in a failing
// connection stage, or connection termination error.
//
// These may be used to specifically test the ports that could have caused the
// connection failure. If no ports are likely involved with a given failure,
...
// Populates the output buffer with a stringified list of the port flags set in
// the input argument. The second and subsequent entries will be prepended by
// 'separator' (if provided). If the output buffer is too small, the output will
// be truncated to fit the provided buffer.
void LiStringifyPortFlags(unsigned int portFlags, const char *separator,
                          char *outputBuffer, int outputBufferLength);

// This function may be used to test if the local network is blocking
// Moonlight's ports. It requires a test server running on an Internet-reachable
// host. To perform a test, pass in the DNS hostname of the test server, a
// reference TCP port to ensure the test host is reachable at all (something
// very unlikely to blocked, like 80 or 443), and a set of ML_PORT_FLAG_* values
// corresponding to the ports you'd like to test. On return, it returns
// ML_TEST_RESULT_INCONCLUSIVE on catastrophic error, or the set of port flags
// that failed to validate. If all ports validate successfully, it returns 0.
//
// It's encouraged to not use the port flags explicitly (because GameStream
// ports may change in the future), but to instead use ML_PORT_FLAG_ALL or
// LiGetPortFlagsFromStage() on connection failure.
//
// The test server is available at https://github.com/cgutman/gfe-loopback
#define ML_TEST_RESULT_INCONCLUSIVE 0xFFFFFFFF
unsigned int LiTestClientConnectivity(const char *testServer,
                                      unsigned short referencePort,
                                      unsigned int testPortFlags);

// This family of functions can be used for pull-based video renderers that opt
// to manage a decoding/rendering thread themselves. After successfully calling
// the WaitFor/Poll variants that dequeue the video frame, you must call
// LiCompleteVideoFrame() to notify that processing is completed. The same DR_*
// status values from drSubmitDecodeUnit() must be passed to
// LiCompleteVideoFrame() as the drStatus argument.
//
// In order to safely use these functions, you must set CAPABILITY_PULL_RENDERER
// on the video decoder.
typedef void *VIDEO_FRAME_HANDLE;
bool LiWaitForNextVideoFrame(VIDEO_FRAME_HANDLE *frameHandle,
                             PDECODE_UNIT *decodeUnit);
bool LiPollNextVideoFrame(VIDEO_FRAME_HANDLE *frameHandle,
                          PDECODE_UNIT *decodeUnit);
...
// This function populates the provided mastering metadata struct with the HDR
// metadata from the host PC's monitor and content (if available). It is only
// valid to call this function when HDR mode is active on the host. This is a
// Sunshine protocol extension.
bool LiGetHdrMetadata(PSS_HDR_METADATA metadata);

// This function requests an IDR frame from the host. Typically this is done
// using DR_NEED_IDR, but clients processing frames asynchronously may need to
// reset their decoder state even after returning DR_OK for the prior frame.
// Rather than wait for a new frame and return DR_NEED_IDR for that one, they
// can just call this API instead. Note that this function does not guarantee
// that the *next* frame will be an IDR frame, just that an IDR frame will
// arrive soon.
void LiRequestIdrFrame(void);

// This function returns any extended feature flags supported by the host.
#define LI_FF_PEN_TOUCH_EVENTS                                                 \
