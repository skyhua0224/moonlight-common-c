...
// called multiple times for retransmissions to work correctly. It is meant to
// be a drop-in replacement for enet_host_service(). It also handles
// cancellation of the connection attempt during the wait.
static int serviceEnetHostInternal(ENetHost *client, ENetEvent *event,
                                   enet_uint32 timeoutMs,
                                   bool ignoreInterrupts) {
  int ret;

  // Clear the last socket error to ensure the caller doesn't read a stale error
  // upon a failure in non-socket-related processing in enet_host_service()
  SetLastSocketError(0);

  // We need to call enet_host_service() multiple times to make sure
  // retransmissions happen
  for (;;) {
    int selectedTimeout = timeoutMs < ENET_INTERNAL_TIMEOUT_MS
                              ? timeoutMs
                              : ENET_INTERNAL_TIMEOUT_MS;
...
// This function performs a graceful disconnect, including lingering until
// outbound traffic is acked (up until the linger timeout elapses).
int gracefullyDisconnectEnetPeer(ENetHost *host, ENetPeer *peer,
                                 enet_uint32 lingerTimeoutMs) {
  // If the connection is being interrupted (e.g. user requested
  // disconnect/quit), don't spend time waiting for graceful ACKs. This avoids
  // UI-visible stalls on exit when the peer or network is already unhealthy.
  if (ConnectionInterrupted) {
    Limelog("Connection interrupted; performing abortive ENet disconnect\n");
    enet_peer_disconnect_now(peer, 0);
    return 0;
  }

  // Check if this peer is currently alive. We won't get another
  // ENET_EVENT_TYPE_DISCONNECT event from ENet if the peer is dead. In that
  // case, we'll do an abortive disconnect.
  if (peer->state == ENET_PEER_STATE_CONNECTED) {
    ENetEvent event;
    int err;

    // Begin the disconnection process. We'll get ENET_EVENT_TYPE_DISCONNECT
    // once the peer acks all outstanding reliable sends.
    enet_peer_disconnect_later(peer, 0);

    // We must use the internal function which lets us ignore pending
    // interrupts.
    while ((err = serviceEnetHostInternal(host, &event, lingerTimeoutMs,
                                          true)) > 0) {
      switch (event.type) {
