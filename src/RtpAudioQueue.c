...
void RtpaInitializeQueue(PRTP_AUDIO_QUEUE queue) {
  ...
  // Older versions of GFE violate some invariants that our FEC code requires,
  // so we turn it off for anything older than GFE 3.19 just to be safe. GFE
  // seems to have changed to the "modern" behavior between GFE 3.18 and 3.19.
  //
  // In the case of GFE 3.13, it does send FEC packets but it requires very
  // special handling because: a) data and FEC shards may vary in size b) FEC
  // blocks can start on boundaries that are not multiples of RTPA_DATA_SHARDS
  //
  // It doesn't seem worth it to sink a bunch of hours into figure out how to
  // properly handle audio FEC for a 3 year old version of GFE that almost
  // nobody uses. Instead, we'll just disable the FEC queue entirely and pass
  // all audio data straight to the decoder.
  //
  if (!APP_VERSION_AT_LEAST(7, 1, 415)) {
    Limelog("Audio FEC has been disabled due to an incompatibility with your "
            "host's old software.\n");
    Limelog("Audio quality may suffer on unreliable network connections due to "
            "lack of FEC!\n");
...
static void validateFecBlockState(PRTP_AUDIO_QUEUE queue) {
  ...
  // The next sequence number must not be less than the oldest BSN unless we're
  // still synchronizing with the source
  LC_ASSERT(!isBefore16(queue->nextRtpSequenceNumber,
  ...
  // The next sequence number must not exceed the first FEC block (otherwise it
  // should have been dequeued and freed)
  LC_ASSERT(isBefore16(queue->nextRtpSequenceNumber,
                       queue->blockHead->fecHeader.baseSequenceNumber +
                           RTPA_DATA_SHARDS));

  // The first FEC block should not be before the oldest BSN (or we will drop
  // packets that belong in that FEC block).
  LC_ASSERT(!isBefore16(queue->blockHead->fecHeader.baseSequenceNumber,
                        queue->oldestRtpBaseSequenceNumber));
  ...
  while (block != NULL) {
    ...
    LC_ASSERT_VT(block->fecHeader.payloadType ==
                 lastBlock->fecHeader.payloadType);
...
static PRTPA_FEC_BLOCK allocateFecBlock(PRTP_AUDIO_QUEUE queue,
                                        uint16_t blockSize) {
  ...
  return malloc(sizeof(*block) + (RTPA_DATA_SHARDS * dataPacketSize) +
                (RTPA_FEC_SHARDS * blockSize));
}
...
static void freeFecBlockHead(PRTP_AUDIO_QUEUE queue) {
  ...
  queue->oldestRtpBaseSequenceNumber =
      blockHead->fecHeader.baseSequenceNumber + RTPA_DATA_SHARDS;

  // Once we complete an FEC block (successfully or not), we're synchronized
  // with the source
...
static PRTPA_FEC_BLOCK getFecBlockForRtpPacket(PRTP_AUDIO_QUEUE queue,
                                               PRTP_PACKET packet,
                                               uint16_t length) {
  ...
  if (packet->packetType == RTP_PAYLOAD_TYPE_AUDIO) {
    ...
    if (!queue->synchronizing &&
        isBefore16(packet->sequenceNumber,
                   queue->oldestRtpBaseSequenceNumber)) {
      queue->lastOosSequenceNumber = packet->sequenceNumber;
      if (!queue->receivedOosData) {
        Limelog(
            "Leaving fast audio recovery mode after OOS audio data (%u < %u)\n",
            packet->sequenceNumber, queue->oldestRtpBaseSequenceNumber);
        queue->receivedOosData = true;
      }
    }
    // This condition looks odd, but it's just a simple way to check if we've
    // gone more than 32767 packets without an OOS packet.
    ...
    fecBlockBaseSeqNum =
        (packet->sequenceNumber / RTPA_DATA_SHARDS) * RTPA_DATA_SHARDS;
    fecBlockBaseTs =
        packet->timestamp -
        ((packet->sequenceNumber - fecBlockBaseSeqNum) * AudioPacketDuration);
    ...
    if (fecBlockBaseSeqNum % RTPA_DATA_SHARDS != 0) {
      // The FEC blocks must start on a RTPA_DATA_SHARDS boundary for our
      // queuing logic to work. This isn't the case for older versions of
      // GeForce Experience (at least 3.13). Disable the FEC logic if this
  ...
  // Synchronize the nextRtpSequenceNumber and oldestRtpBaseSequenceNumber
  // values when the connection begins. Start on the next FEC block boundary, so
  // we can be sure we aren't starting in the middle (which will lead to a
  // spurious audio data block recovery warning on connection start if we miss
  // more than 2 packets).
  if (queue->synchronizing && queue->oldestRtpBaseSequenceNumber == 0) {
    queue->nextRtpSequenceNumber = queue->oldestRtpBaseSequenceNumber =
        fecBlockBaseSeqNum + RTPA_DATA_SHARDS;
    return NULL;
  }
  ...
  while (existingBlock != NULL) {
    if (existingBlock->fecHeader.baseSequenceNumber == fecBlockBaseSeqNum) {
      ...
      if (existingBlock->blockSize != blockSize) {
        // This can happen with older versions of GeForce Experience (3.13) and
        // Sunshine that don't use a constant size for audio packets.
        Limelog("Audio block size mismatch (got %u, expected %u)\n", blockSize,
                existingBlock->blockSize);
        Limelog("Audio FEC has been disabled due to an incompatibility with "
                "your host's old software!\n");
        queue->incompatibleServer = true;
        return NULL;
      }
    ...
    } else if (isBefore16(fecBlockBaseSeqNum,
                          existingBlock->fecHeader.baseSequenceNumber)) {
...
static bool completeFecBlock(PRTP_AUDIO_QUEUE queue, PRTPA_FEC_BLOCK block) {
  ...
  if (block->dataShardsReceived + block->fecShardsReceived <
      RTPA_DATA_SHARDS + 1) {
#else
  ...
  memcpy(droppedRtpPacket, block->dataPackets[dropIndex],
         sizeof(RTP_PACKET) + block->blockSize);

  // Fake the drop by setting the mark bit and zeroing the "missing" packet
  block->marks[dropIndex] = 1;
  memset(block->dataPackets[dropIndex], 0,
         sizeof(RTP_PACKET) + block->blockSize);
#endif

  int res = reed_solomon_reconstruct(queue->rs, shards, block->marks,
                                     RTPA_TOTAL_SHARDS, block->blockSize);
  if (res != 0) {
    // We should always have enough data to recover the entire block since we
    // checked above.
    LC_ASSERT(res == 0);
    return false;
  }
  ...
  for (int i = 0; i < RTPA_DATA_SHARDS; i++) {
    if (block->marks[i]) {
      ...
      block->dataPackets[i]->sequenceNumber =
          block->fecHeader.baseSequenceNumber + i;
      block->dataPackets[i]->timestamp =
          block->fecHeader.baseTimestamp + (i * AudioPacketDuration);
  ...
  LC_ASSERT_VT(block->dataPackets[dropIndex]->header ==
               droppedRtpPacket->header);
  LC_ASSERT_VT(block->dataPackets[dropIndex]->packetType ==
               droppedRtpPacket->packetType);
  LC_ASSERT_VT(block->dataPackets[dropIndex]->sequenceNumber ==
               droppedRtpPacket->sequenceNumber);
  LC_ASSERT_VT(block->dataPackets[dropIndex]->timestamp ==
               droppedRtpPacket->timestamp);
  ...
  if (memcmp(block->dataPackets[dropIndex] + 1, droppedRtpPacket + 1,
             block->blockSize)) {
    unsigned char *actualData =
        (unsigned char *)(block->dataPackets[dropIndex] + 1);
...
static bool queueHasPacketReady(PRTP_AUDIO_QUEUE queue) {
  ...
  return queue->blockHead != NULL &&
         ((queue->blockHead->marks[queue->blockHead->nextDataPacketIndex] ==
               0 &&
           queue->blockHead->fecHeader.baseSequenceNumber +
                   queue->blockHead->nextDataPacketIndex ==
...
static void handleMissingPackets(PRTP_AUDIO_QUEUE queue) {
  ...
  // If the packet we're waiting on precedes our earliest FEC block, a previous
  // FEC block was completely lost. We should resynchronize immediately by
  // advancing the queue state to play our oldest block next.
  //
  // NB: We do NOT want to set allowDiscontinuity here, because that will result
  // in playing back the entire FEC block immediately but we've only received a
  // single packet from that block. Worse still, when the remaining packets from
  // this block arrive, they will trigger the OOS detection and kick us out of
  // fast audio recovery mode.
  if (isBefore16(queue->nextRtpSequenceNumber,
                 queue->blockHead->fecHeader.baseSequenceNumber)) {
    queue->nextRtpSequenceNumber =
        queue->blockHead->fecHeader.baseSequenceNumber;
    queue->oldestRtpBaseSequenceNumber =
  ...
  // If we reach this point, we know the next packet resides in the first FEC
  // block we're currently waiting on. In that case, we want to wait at least
  // until we have a second FEC block to give up on the first one. If we don't
  // have a second block now, just keep waiting.
  LC_ASSERT_VT(isBefore16(queue->nextRtpSequenceNumber,
                          queue->blockHead->fecHeader.baseSequenceNumber +
                              RTPA_DATA_SHARDS));
  ...
  // At this point, we know we've got a second FEC block queued up waiting on
  // the first one to complete. If we've never seen OOS data from this host,
  // we'll assume the first one is lost and skip forward. If we have seen OOS
  // data, we'll wait for a little while longer to see if OOS packets arrive
  // before giving up.
  if (!queue->receivedOosData ||
      PltGetMillis() - queue->blockHead->queueTimeMs >
          (uint32_t)(AudioPacketDuration * RTPA_DATA_SHARDS) +
              RTPQ_OOS_WAIT_TIME_MS) {
    LC_ASSERT(!isBefore16(queue->nextRtpSequenceNumber,
                          queue->blockHead->fecHeader.baseSequenceNumber));

    Limelog("Unable to recover audio data block %u to %u (%u+%u=%u received < "
            "%u needed)\n",
            queue->blockHead->fecHeader.baseSequenceNumber,
            queue->blockHead->fecHeader.baseSequenceNumber + RTPA_DATA_SHARDS -
                1,
...
int RtpaAddPacket(PRTP_AUDIO_QUEUE queue, PRTP_PACKET packet, uint16_t length) {
  if (queue->incompatibleServer) {
    // Just feed audio data straight through to the decoder. We lose handling of
    // out-of-order and duplicated packets in this mode, but it shouldn't be a
    // problem for the very small portion of users that are running an ancient
    // GFE or Sunshine version.
    if (packet->packetType == RTP_PAYLOAD_TYPE_AUDIO) {
  ...
  if (packet->packetType == RTP_PAYLOAD_TYPE_AUDIO) {
    ...
    if (fecBlock->marks[pos]) {
      // If there was a missing data shard, copy the RTP header and packet data
      // into it
    ...
    if (packet->sequenceNumber == queue->nextRtpSequenceNumber) {
      ...
      if (queue->nextRtpSequenceNumber ==
          U16(fecBlock->fecHeader.baseSequenceNumber + RTPA_DATA_SHARDS)) {
  ...
  LC_ASSERT(fecBlock == queue->blockHead ||
            queue->blockHead != queue->blockTail);
...
PRTP_PACKET RtpaGetQueuedPacket(PRTP_AUDIO_QUEUE queue,
                                uint16_t customHeaderLength, uint16_t *length) {
  validateFecBlockState(queue);

  // If we're returning audio data even with discontinuities, we'll fill in
  // blank entries for packets that were lost and could not be recovered.
  if (queue->blockHead != NULL && queue->blockHead->allowDiscontinuity) {
    ...
    LC_ASSERT(nextBlock->fecHeader.baseSequenceNumber +
  ...
  // Return the next RTP sequence number by indexing into the most recent FEC
  // block
  if (queueHasPacketReady(queue)) {
    PRTPA_FEC_BLOCK nextBlock = queue->blockHead;
    PRTP_PACKET packet =
        malloc(customHeaderLength + sizeof(RTP_PACKET) + nextBlock->blockSize);
    if (packet == NULL) {
      return NULL;
    }

    *length = nextBlock->blockSize + sizeof(RTP_PACKET);
    memcpy((uint8_t *)packet + customHeaderLength,
           nextBlock->dataPackets[nextBlock->nextDataPacketIndex], *length);
    nextBlock->nextDataPacketIndex++;
