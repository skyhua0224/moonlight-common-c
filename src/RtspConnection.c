...
#define sessionIdString (rtsp->sessionIdString)
#define hasSessionId (rtsp->hasSessionId)
#define rtspClientVersion (rtsp->rtspClientVersion)
#define urlAddr (rtsp->urlAddr)
#define useEnet (rtsp->useEnet)
#define controlStreamId (rtsp->controlStreamId)
#define encryptedRtspEnabled (rtsp->encryptedRtspEnabled)
#define encryptionCtx (rtsp->encryptionCtx)
#define decryptionCtx (rtsp->decryptionCtx)
#define encryptionSequenceNumber (rtsp->encryptionSequenceNumber)
#define sock (rtsp->sock)
#define client (rtsp->client)
#define peer (rtsp->peer)

// Macros to redirect global access to connection context
...
static bool initializeRtspRequest(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE msg, char *command,
                                  char *target) {
  char sequenceNumberStr[16];
  char clientVersionStr[16];

  // FIXME: Hacked CSeq attribute due to RTSP parser bug
  createRtspRequest(msg, NULL, 0, command, target, "RTSP/1.0", 0, NULL, NULL,
                    0);

  snprintf(sequenceNumberStr, sizeof(sequenceNumberStr), "%d",
           currentSeqNumber++);
...
static char *sealRtspMessage(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE request, int *messageLen) {
  ...
  encryptedMessage =
      (PENC_RTSP_HEADER)malloc(sizeof(ENC_RTSP_HEADER) + plaintextLen);
  ...
  success =
      PltEncryptMessage(encryptionCtx, ALGORITHM_AES_GCM, 0,
                        (uint8_t *)StreamConfig.remoteInputAesKey,
...
static bool unsealRtspMessage(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, char *rawMessage, int rawMessageLen,
                              PRTSP_MESSAGE response) {
  ...
  if (encryptedRtspEnabled) {
    ...
    success = PltDecryptMessage(
        decryptionCtx, ALGORITHM_AES_GCM, 0,
        (uint8_t *)StreamConfig.remoteInputAesKey,
        sizeof(StreamConfig.remoteInputAesKey), iv, sizeof(iv),
        encryptedMessage->tag, sizeof(encryptedMessage->tag),
  ...
  if (parseRtspMessage(response, decryptedMessage, decryptedMessageLen) ==
      RTSP_ERROR_SUCCESS) {
    success = true;
  } else {
...
static bool transactRtspMessageEnet(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE request,
                                    PRTSP_MESSAGE response,
                                    bool expectingPayload, int *error) {
  ENetEvent event;
  char *serializedMessage;
  int messageLen;
  int offset;
  ...
  *error = -1;
  ...
  // We're going to handle the payload separately, so temporarily set the
  // payload to NULL
  ...
  packet = enet_packet_create(serializedMessage, messageLen,
  ...
  if (payload != NULL) {
    packet =
        enet_packet_create(payload, payloadLength, ENET_PACKET_FLAG_RELIABLE);
    if (packet == NULL) {
      goto Exit;
    }
  ...
  if (expectingPayload) {
    ...
    // Copy the payload out to the end of the response buffer and destroy the
    // packet
    memcpy(&responseBuffer[offset], event.packet->data,
           event.packet->dataLength);
    offset += (int)event.packet->dataLength;
    enet_packet_destroy(event.packet);
  }

  if (parseRtspMessage(response, responseBuffer, offset) ==
      RTSP_ERROR_SUCCESS) {
...
static bool transactRtspMessageTcp(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE request,
                                   PRTSP_MESSAGE response, int *error) {
  SOCK_RET err;
  bool ret;
  int offset;
  ...
  *error = -1;
  ...
  } while (connectRetries++ <
               (RTSP_CONNECT_TIMEOUT_SEC * 1000) / RTSP_RETRY_DELAY_MS &&
           !ConnectionInterrupted);
  if (sock == INVALID_SOCKET) {
    return ret;
  }

  serializedMessage = sealRtspMessage(ctx, rtsp, request, &messageLen);
  if (serializedMessage == NULL) {
    closeSocket(sock);
    sock = INVALID_SOCKET;
    return ret;
  ...
  ret = unsealRtspMessage(ctx, rtsp, responseBuffer, offset, response);
...
static bool transactRtspMessage(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE request, PRTSP_MESSAGE response,
                                bool expectingPayload, int *error) {
  ...
  if (useEnet) {
    return transactRtspMessageEnet(ctx, rtsp, request, response, expectingPayload, error);
  } else {
    return transactRtspMessageTcp(ctx, rtsp, request, response, error);
  }
}

// Send RTSP OPTIONS request
static bool requestOptions(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE response, int *error) {
  RTSP_MESSAGE request;
  bool ret;

  *error = -1;

  ret = initializeRtspRequest(ctx, rtsp, &request, "OPTIONS", rtspTargetUrl);
  if (ret) {
    ret = transactRtspMessage(ctx, rtsp, &request, response, false, error);
    freeMessage(&request);
  }
...
static bool requestDescribe(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE response, int *error) {
  RTSP_MESSAGE request;
  bool ret;

  *error = -1;

  ret = initializeRtspRequest(ctx, rtsp, &request, "DESCRIBE", rtspTargetUrl);
  if (ret) {
    if (addOption(&request, "Accept", "application/sdp") &&
        addOption(&request, "If-Modified-Since",
                  "Thu, 01 Jan 1970 00:00:00 GMT")) {
      ret = transactRtspMessage(ctx, rtsp, &request, response, true, error);
    } else {
      ret = false;
    }
    freeMessage(&request);
...
static bool setupStream(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE response, char *target, int *error) {
  RTSP_MESSAGE request;
  bool ret;
  char *transportValue;
  ...
  *error = -1;
  ...
  if (ret) {
    ...
    if (ConnectionAppVersionQuad[0] >= 6) {
...
static bool playStream(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE response, char *target, int *error) {
  RTSP_MESSAGE request;
  bool ret;
...
static bool sendVideoAnnounce(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE response, int *error) {
  RTSP_MESSAGE request;
  bool ret;
  int payloadLength;
  ...
  *error = -1;
  ...
  ret = initializeRtspRequest(
      ctx, rtsp, &request, "ANNOUNCE",
      APP_VERSION_AT_LEAST_CTX(ctx, 7, 1, 431) ? controlStreamId : "streamid=video");
  if (ret) {
    ret = false;
    ...
    request.payload =
        getSdpPayloadForStreamConfigCtx(ctx, rtspClientVersion, &payloadLength);
...
static bool parseServerPortFromTransport(PML_CONNECTION_CONTEXT ctx, PRTSP_MESSAGE response,
                                         uint16_t *port) {
  char *transport;
  char *portStart;
...
static int parseOpusConfigurations(PML_CONNECTION_CONTEXT ctx, PRTSP_HANDSHAKE_CONTEXT rtsp, PRTSP_MESSAGE response) {
  ...
  if (CHANNEL_COUNT_FROM_AUDIO_CONFIGURATION(StreamConfig.audioConfiguration) ==
      2) {
    ...
    snprintf(paramsPrefix, sizeof(paramsPrefix), "a=fmtp:97 surround-params=%d",
             channelCount);
    ...
    if (paramStart) {
      ...
      err = parseOpusConfigFromParamString(ctx, paramStart, channelCount,
      ...
      if (channelCount == 6 || channelCount == 8) {
        ...
        // LFE comes after C
        NormalQualityOpusConfig.mapping[3] =
      ...
      // It's unknown whether all GFE versions that supported surround sound
      // included these surround sound parameters. In case they didn't, we'll
      // specifically handle 5.1 surround sound using a hardcoded configuration
      // like we used to before this parsing code existed.
      //
      // It is not necessary to provide HighQualityOpusConfig here because high
      // quality mode can only be enabled from seeing the required
      // "surround-params=" value, so there's no chance it could regress from
      // implementing this parser.
      if (channelCount == 6) {
...
static bool parseUrlAddrFromRtspUrlString(const char *rtspUrlString,
                                          char *destination,
  ...
  for (prefixLen = 2; rtspUrlScratchBuffer[prefixLen - 2] != 0 &&
                      (rtspUrlScratchBuffer[prefixLen - 2] != '/' ||
                       rtspUrlScratchBuffer[prefixLen - 1] != '/');
       prefixLen++)
    ;

  // If we hit the end of the string prior to parsing the prefix, we cannot
  // proceed
  ...
  // Look for a slash at the end of the host portion of the URL (may not be
  // present)
  ...
  if (!PltSafeStrcpy(destination, destinationLength,
                     rtspUrlScratchBuffer + prefixLen)) {
    free(rtspUrlScratchBuffer);
    return false;
  }
...
bool parseSdpAttributeToUInt(const char *payload, const char *name,
  ...
  *val = (unsigned int)strtoul(valst + 1, NULL, 0);
...
bool parseSdpAttributeToInt(const char *payload, const char *name, int *val) {
  ...
  // Read the integer up to the newline at the end of the SDP attribute
  *val = (int)strtol(valst + 1, NULL, 0);

  return true;
}

// Perform RTSP Handshake with the streaming server machine as part of the
// connection process
int performRtspHandshakeCtx(PML_CONNECTION_CONTEXT ctx, PSERVER_INFORMATION serverInfo) {
  int ret;
  RTSP_HANDSHAKE_CONTEXT rtspStack;
  PRTSP_HANDSHAKE_CONTEXT rtsp = &rtspStack;
  ...
  useEnet = (ConnectionAppVersionQuad[0] >= 5) && (ConnectionAppVersionQuad[0] <= 7) &&
            (ConnectionAppVersionQuad[2] < 404);
  currentSeqNumber = 1;
  hasSessionId = false;
  controlStreamId = APP_VERSION_AT_LEAST(7, 1, 431) ? "streamid=control/13/0"
                                                    : "streamid=control/1/0";
  AudioEncryptionEnabled = false;
  MicPortNumber = 0;
  encryptedRtspEnabled = serverInfo->rtspSessionUrl &&
                         strstr(serverInfo->rtspSessionUrl, "rtspenc://");
  encryptionCtx = PltCreateCryptoContext();
  decryptionCtx = PltCreateCryptoContext();

  // HACK: In order to get GFE to respect our request for a lower audio bitrate,
  // we must fake our target address so it doesn't match any of the PC's local
  // interfaces. It seems that the only way to get it to give you "low quality"
  // stereo audio nowadays is if it thinks you are remote (target address != any
  // local address).
  //
  // We will enable high quality audio if the following are all true:
  // 1. Video bitrate is higher than 15 Mbps (to ensure most bandwidth is
  // reserved for video)
  // 2. The audio decoder has not declared that it is slow
  // 3. The stream is either local or not surround sound (to prevent MTU issues
  // over the Internet)
  ...
  if (StreamConfig.bitrate >= HIGH_AUDIO_BITRATE_THRESHOLD &&
      ...
      (StreamConfig.streamingRemotely != STREAM_CFG_REMOTE ||
       CHANNEL_COUNT_FROM_AUDIO_CONFIGURATION(
           StreamConfig.audioConfiguration) <= 2)) {
    // If we have an RTSP URL string and it was successfully parsed and copied,
    // use that string
    if (serverInfo->rtspSessionUrl == NULL ||
        !parseUrlAddrFromRtspUrlString(serverInfo->rtspSessionUrl, urlAddr,
                                       sizeof(urlAddr)) ||
        !PltSafeStrcpy(rtspTargetUrl, sizeof(rtspTargetUrl),
                       serverInfo->rtspSessionUrl)) {
      // If an RTSP URL string was not provided or failed to parse, we will
      // construct one now as best we can.
      //
      // NB: If the remote address is not a LAN address, the host will likely
      // not enable high quality audio since it only does that for local
      // streaming normally. We can avoid this limitation, but only if the
      // caller gave us the RTSP session URL that it received from the host
      // during launch.
      addrToUrlSafeString(&RemoteAddr, urlAddr, sizeof(urlAddr));
      snprintf(rtspTargetUrl, sizeof(rtspTargetUrl), "rtsp%s://%s:%u",
               useEnet ? "ru" : "", urlAddr, RtspPortNumber);
    }
  } else {
  ...
  switch (ConnectionAppVersionQuad[0]) {
  case 3:
...

  {
    ...
    if (!requestOptions(ctx, rtsp, &response, &error)) {
...

  {
    ...
    if (!requestDescribe(ctx, rtsp, &response, &error)) {
    ...
    if ((StreamConfig.supportedVideoFormats & VIDEO_FORMAT_MASK_AV1) &&
        strstr(response.payload, "AV1/90000")) {
      if ((serverInfo->serverCodecModeSupport & SCM_AV1_HIGH10_444) &&
          (StreamConfig.supportedVideoFormats & VIDEO_FORMAT_AV1_HIGH10_444)) {
        NegotiatedVideoFormat = VIDEO_FORMAT_AV1_HIGH10_444;
      } else if ((serverInfo->serverCodecModeSupport & SCM_AV1_MAIN10) &&
                 (StreamConfig.supportedVideoFormats &
                  VIDEO_FORMAT_AV1_MAIN10)) {
        NegotiatedVideoFormat = VIDEO_FORMAT_AV1_MAIN10;
      } else if ((serverInfo->serverCodecModeSupport & SCM_AV1_HIGH8_444) &&
                 (StreamConfig.supportedVideoFormats &
    ...
    } else if ((StreamConfig.supportedVideoFormats & VIDEO_FORMAT_MASK_H265) &&
               strstr(response.payload, "sprop-parameter-sets=AAAAAU")) {
      // The RTSP DESCRIBE reply will contain a collection of SDP media
      // attributes that describe the various supported video stream formats and
      // include the SPS, PPS, and VPS (if applicable). We will use this
      // information to determine whether the server can support HEVC. For some
      // reason, they still set the MIME type of the HEVC format to H264, so we
      // can't just look for the HEVC MIME type. What we'll do instead is look
      // for the base 64 encoded VPS NALU prefix that is unique to the HEVC
      // bitstream.
      if ((serverInfo->serverCodecModeSupport & SCM_HEVC_REXT10_444) &&
          (StreamConfig.supportedVideoFormats & VIDEO_FORMAT_H265_REXT10_444)) {
        NegotiatedVideoFormat = VIDEO_FORMAT_H265_REXT10_444;
      } else if ((serverInfo->serverCodecModeSupport & SCM_HEVC_MAIN10) &&
                 (StreamConfig.supportedVideoFormats &
                  VIDEO_FORMAT_H265_MAIN10)) {
        NegotiatedVideoFormat = VIDEO_FORMAT_H265_MAIN10;
      } else if ((serverInfo->serverCodecModeSupport & SCM_HEVC_REXT8_444) &&
                 (StreamConfig.supportedVideoFormats &
      ...
      if ((serverInfo->serverCodecModeSupport & SCM_H264_HIGH8_444) &&
          (StreamConfig.supportedVideoFormats & VIDEO_FORMAT_H264_HIGH8_444)) {
      ...
      if (StreamConfig.width > 4096 || StreamConfig.height > 4096) {
        Limelog("WARNING: Host PC doesn't support HEVC. Streaming at "
    ...
    if (!parseSdpAttributeToUInt(response.payload, "x-ss-general.featureFlags",
                                 &SunshineFeatureFlags)) {
      SunshineFeatureFlags = 0;
    }

    // Look for the Sunshine encryption flags in the SDP attributes
    if (!parseSdpAttributeToUInt(response.payload,
    ...
    ret = parseOpusConfigurations(ctx, rtsp, &response);
    if (ret != 0) {
      goto Exit;
    }
...

  {
    ...
    if (!setupStream(ctx, rtsp, &response,
             ConnectionAppVersionQuad[0] >= 5 ? "streamid=audio/0/0"
                                            : "streamid=audio",
                     &error)) {
    ...
    if (!parseServerPortFromTransport(ctx, &response, &AudioPortNumber)) {
    ...
    if (pingPayload != NULL &&
        strlen(pingPayload) == sizeof(AudioPingPayload.payload)) {
      memcpy(AudioPingPayload.payload, pingPayload,
             sizeof(AudioPingPayload.payload));
    }
...

  {
    ...
    if (!setupStream(ctx, rtsp, &response,
             ConnectionAppVersionQuad[0] >= 5 ? "streamid=video/0/0"
                                            : "streamid=video",
                     &error)) {
    ...
    pingPayload = getOptionContent(response.options, "X-SS-Ping-Payload");
    if (pingPayload != NULL &&
        strlen(pingPayload) == sizeof(VideoPingPayload.payload)) {
      memcpy(VideoPingPayload.payload, pingPayload,
             sizeof(VideoPingPayload.payload));
    }

    // Parse the video port out of the RTSP SETUP response
    LC_ASSERT(VideoPortNumber == 0);
    if (!parseServerPortFromTransport(ctx, &response, &VideoPortNumber)) {
      // Use the well known port if parsing fails
  ...
  // Setup microphone stream if requested, before control stream to maintain
  // logical order
  if (StreamConfig.enableMic) {
    ...
    if (!setupStream(ctx, rtsp, &response,
             ConnectionAppVersionQuad[0] >= 5 ? "streamid=mic/0/0"
                                            : "streamid=mic",
                     &error)) {
    ...
    if (!parseServerPortFromTransport(ctx, &response, &MicPortNumber)) {
      // Use the well known port if parsing fails
      MicPortNumber = 47996;
    ...
    pingPayload = getOptionContent(response.options, "X-SS-Ping-Payload");
    if (pingPayload != NULL &&
        strlen(pingPayload) == sizeof(MicPingPayload.payload)) {
      memcpy(MicPingPayload.payload, pingPayload,
             sizeof(MicPingPayload.payload));
    }

    freeMessage(&response);
  }

  if (ConnectionAppVersionQuad[0] >= 5) {
    RTSP_MESSAGE response;
    int error = -1;
    char *connectData;

    if (!setupStream(ctx, rtsp, &response, controlStreamId, &error)) {
    ...
    if (!parseServerPortFromTransport(ctx, &response, &ControlPortNumber)) {
      // Use the well known port if parsing fails
      ControlPortNumber = (uint16_t)47999;
...

  {
    ...
    if (!sendVideoAnnounce(ctx, rtsp, &response, &error)) {
  ...
  if (APP_VERSION_AT_LEAST(7, 1, 431)) {
    ...
    if (!playStream(ctx, rtsp, &response, "/", &error)) {
  ...
  } else {
    {
      ...
      if (!playStream(ctx, rtsp, &response, "streamid=video", &error)) {
...

    {
      ...
      if (!playStream(ctx, rtsp, &response, "streamid=audio", &error)) {
    ...
    if (StreamConfig.enableMic) {
      ...
      if (!playStream(ctx, rtsp, &response, "streamid=mic", &error)) {
